...
void MATRIZ::hacia(char* Fmat){

   // Graba un vector binario
 ...
 ofstream Out;
   ...
   for(int i=0;i<ren;i++){
      Out.write((char*)&celda[i][0], sizeof (double));;
   }
...
MATRIZ MATRIZ::GaussSeidel(MATRIZ &A, MATRIZ &B,double relax, int iter){
   ...
   for(int it=1;it<=iter;it++){
      ...
      if(it%500==0){
         cout<<" GSeidel Error en iteracion "<<it<<" |Ax-B|(1) = "<<(A*X2-B).norma1()<<endl;
         string Fnom;
         Fnom="GSEI";
         Fnom+=it;
         Fnom+=".bin";
...
void MATRIZ::inversa(const MATRIZ &A){
   ...
   for(int j=0;j<col;j++){
      ...
      for(int i=0;i<ren;i++){
         ...
         if(i==ren)
            continue;
         RENGLONsuma(i,B.celda[j][j],j,B.celda[i][j]);
