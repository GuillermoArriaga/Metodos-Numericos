...
// Eliminacion Gaussiana
   ...
   void truncar(int tr=2,bool diagonal=1);
...
// Inversa de A (sin pivoteo)
   MATRIZ inversa();
   MATRIZ transpuesta();
   void NewCol(const MATRIZ &Vec,int numcol); // Modifica una columna
   void Ortonormalizacion();
   MATRIZ GetCol(int numcol);             // Obtiene una columna como vector

   // Calcula diagonal de eigenvalores
   void EigenValor(MATRIZ &A, MATRIZ &EigenVecColumna);
   MATRIZ EigenVectorON(MATRIZ &A,bool tipo=1);
   MATRIZ EigenVectorON2(MATRIZ &A,bool tipo=1);

   // Eigenvectores metodo de potencia
   void EVec(const MATRIZ &A);
   void SVD(MATRIZ &U, MATRIZ &S,MATRIZ &V,bool impresion=0);
   MATRIZ reducida(int NR1,int NC1);
